...
Manuals was hard, but I had to alter the parameterized constructor and that was all.
*/
#include "Manual.hpp"
...
Manual::Manual(std::string name, std::string author, int page_count, std::string model, bool is_digital, std::string url, bool aid) : Book(name, author, page_count, is_digital), has_visual_aid_{aid}, device_{model}
{
  ...
  {
    website_="NONE";//checks whether or not the link is NONE
    has_website_ = false;
  }
  else {
    website_ = "Broken Link";//everything else is broken link
    has_website_ = false;
  }
}
...
std::cout<<title_<<" is written by "<<author_<<" company for device: "<<device_.substr(0,device_.find("-"))<<". Website: "<<website_<<". Page Count: "<<page_count_<<". "<<digital<<" available digitally.\n";
}
